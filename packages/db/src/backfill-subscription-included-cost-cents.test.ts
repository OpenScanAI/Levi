...
describeEmbeddedPostgres("0095_backfill_subscription_included_cost_cents", () => {
  it(
    ...
    async () => {
      ...
      try {
        const backfillHash = await migrationHash("0095_backfill_subscription_included_cost_cents.sql");
        await sql.unsafe(
