...
Commands:
  ...
  share <issue-id>             Generate a shareable preview URL for an issue's workspace
"""
...
def generate_shareable_url(issue_id):
    """Generate a shareable preview URL for an issue's workspace."""
    # Get issue details
    issue = api("GET", f"/issues/{issue_id}")
    if 'error' in issue:
        issue = api("GET", f"/companies/{COMPANY}/issues/{issue_id}")
    ...
    if not project_type:
        print(f"Could not detect project type in {cwd}")
        return None
    
    # Find a free port
    port = find_free_port(PROJECT_TYPES[project_type]["default_port"])
    if not port:
        print("Could not find a free port")
        return None
    ...
    # Try to get the public URL from environment or config
    public_url = os.environ.get('PAPERCLIP_PUBLIC_URL', '')
    if not public_url:
        # Try to get from instance settings
        settings = api("GET", "/instance-settings")
        if isinstance(settings, dict):
            public_url = settings.get('publicBaseUrl', '')
    
    # Generate shareable link
    share_url = f"{public_url}/preview/{issue_id}" if public_url else local_url
    ...
    result = {
        ...
        "share_url": share_url,
        "port": port,
        "agent_name": issue.get('assigneeAgentId', 'unassigned'),
        "issue_title": issue.get('title', 'Untitled'),
    }
    
    # Print to chat/terminal
    print("\n" + "="*60)
    print("🔗 SHAREABLE PREVIEW LINK")
    print("="*60)
    print(f"Issue: {result['issue_title']}")
    print(f"Agent: {result['agent_name']}")
    print(f"Project: {result['project_type']}")
    print(f"\n📎 Shareable URL: {result['share_url']}")
    print(f"🖥️  Local URL: {result['local_url']}")
    print("="*60)
    
    # Also post as comment to the issue
    comment_text = f"🔗 **Shareable Preview Link**\n\n- **URL**: {result['share_url']}\n- **Local**: {result['local_url']}\n- **Project Type**: {result['project_type']}\n\n_Generated by Hermes Bridge_"
    comment = api("POST", f"/companies/{COMPANY}/issues/{issue_id}/comments", {
        "content": comment_text
    })
    if 'error' not in comment:
        print(f"\n✅ Posted preview link to issue {issue_id}")
...
if __name__ == "__main__":
    ...
    if cmd == "list-agents":
        ...
        generate_shareable_url(sys.argv[2])
    else:
